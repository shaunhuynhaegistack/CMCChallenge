...
| **The scenario was racing the browser it was testing.** The back-button scenario drove a navigation — first a reload, then a fresh `goto` — to make the server answer for the restored page. The browser was already going there by itself, and each engine aborted the navigation that collided with its own: `NS_BINDING_ABORTED` on Firefox, `ERR_ABORTED` on Chromium. So the scenario failed on the very redirect it was waiting for, on whichever engine it happened to lose the race. | 66 of 67 — on Firefox in one run and on Chromium in the next, after the first fix moved the race rather than removing it. Probing both engines showed why they differ: Chromium redraws the dashboard from its cache and then leaves when the page finds the session gone; Firefox never restores it at all. It now waits for the browser instead of pushing it, and asserts what is not the browser's decision — the same address, asked for with the same cookies, answers `302` to the login form |
