...
| [`lib/selectors.ts`](lib/selectors.ts) | The locator strategy — why a UI change is one file, not fifteen |
...
| Reusable utilities | `BasePage.clickAndWaitForApi`, `lib/utils/waits.ts`, `lib/utils/data-helper.ts`, `lib/utils/employee-factory.ts`, `lib/utils/date-format.ts`, `lib/selectors.ts`, `lib/api/ApiClient.ts` |
...
| **Centralised selectors** | `lib/selectors.ts` — a design-system change is one file |
...
lib/
  BasePage.ts                  shared interactions and waiting helpers
  selectors.ts                 every application class name, in one place
  ...
  utils/                       data-helper, employee-factory, date-format, waits
...
test-data/                     users, roles, employees, expected messages
...
2. **Everything else** uses the component class, and every one of those lives in
   [`lib/selectors.ts`](lib/selectors.ts). When the design system moves, that
